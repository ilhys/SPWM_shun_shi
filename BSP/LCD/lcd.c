 ...
 * @param       num: floating-point number to be displayed
 * @param       len: number of decimal digits to be displayed
 * @param       size: select font size 12/16/24/32
 * @param       color: color of the displayed digits
 * @retval      None
 */
void lcd_show_float(uint16_t x, uint16_t y, float temp, uint8_t z_len, uint8_t x_len,uint8_t size, uint16_t color)
{
	uint16_t adcx=0;
    adcx = temp;                        /* 赋值整数部分给adcx变量，因为adcx为u16整形 */
    lcd_show_xnum(x, y, adcx, z_len, size, 0, color);   /* 显示电压值的整数部分，3.1111的话，这里就是显示3 */
    temp -= adcx;                       /* 把已经显示的整数部分去掉，留下小数部分，比如3.1111-3=0.1111 */
    temp *= 1000;                       /* 小数部分乘以1000，例如：0.1111就转换为111.1，相当于保留三位小数。 */
    x=x+z_len*9;
	lcd_show_xnum(x, y, temp, x_len, size, 0X80, color);/* 显示小数部分（前面转换为了整形显示），这里显示的就是111. */
